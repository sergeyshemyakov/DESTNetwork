...
os.environ["DEPLOYER"] = d["deployerAddress"]
...
# command = f"forge script script/CampaignManager.s.sol:ManagerScript --chain-id {chain_id} --rpc-url {rpc_url} --verifier blockscout --legacy --verifier-url {verifier_rpc} --broadcast --verify -vvvv"
command = f"forge script script/CampaignManager.s.sol:ManagerScript --chain-id {chain_id} --rpc-url {rpc_url}  --broadcast --legacy -vvvv -g 1000"
print(command)
os.system(command)

# command = f'forge verify-contract 0xeca4Eee1DCEa4eeec1e7741a839C819Bae6635bC --flatten --watch --compiler-version "v0.8.26+commit.8a97fa7a" --verifier-url {verifier_rpc} --api-key {etherscan_key} --optimizer-runs 200 --chain-id {chain_id}' --constructor-args $(cast abi-encode "constructor(uint256,address,uint256,uint256,uint256,address,tuple(int256,int256),tuple(int256,int256),bytes32,address,address,uint256)" "" "FUSD" 18 1000000000000000000000)
# print(command)
# os.system(command)
